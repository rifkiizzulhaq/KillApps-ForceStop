...
class KillerExecutionHelperTest {
    ...
    fun `isSmartProtected returns false immediately when smart is disabled`() {
        val result = KillerExecutionHelper.isSmartProtected(mockContext, "com.any.app", smart = false)
        assertFalse(result)
    }
    ...
    fun `isSmartProtected returns true for critical system packages`() {
        ...
        for (pkg in criticals) {
            assertTrue("$pkg should be protected", KillerExecutionHelper.isSmartProtected(mockContext, pkg, smart = true))
        }
    }
    ...
    fun `isSmartProtected returns false for non-critical app without active call`() {
        every { mockAudioManager.mode } returns AudioManager.MODE_NORMAL
        val result = KillerExecutionHelper.isSmartProtected(mockContext, "com.example.app", smart = true)
        assertFalse(result)
    }
    ...
    fun `isSmartProtected returns true when device is in communication mode`() {
        ...
        val result = KillerExecutionHelper.isSmartProtected(mockContext, "com.voip.app", smart = true)
        assertTrue(result)
    }

    @Test
    fun `isMediaActiveProtected returns false immediately when finerMedia is disabled`() {
        val result = KillerExecutionHelper.isMediaActiveProtected(mockContext, "com.spotify.music", finerMedia = false)
        assertFalse(result)
    }
    ...
    fun `isMediaActiveProtected returns false for non-media app even when music is active`() {
        ...
        val result = KillerExecutionHelper.isMediaActiveProtected(mockContext, "com.example.app", finerMedia = true)
        assertFalse(result)
    }
    ...
    fun `isMediaActiveProtected returns true for known media app when music is active`() {
        every { mockAudioManager.isMusicActive } returns true
        val result = KillerExecutionHelper.isMediaActiveProtected(mockContext, "com.spotify.music", finerMedia = true)
        assertTrue(result)
    }
    ...
    fun `isMediaActiveProtected returns true for dynamic media package when music active`() {
        ...
        val result = KillerExecutionHelper.isMediaActiveProtected(mockContext, "com.custom.player", finerMedia = true, dynamicMediaPkgs = dynamicPkgs)
        assertTrue(result)
    }
    ...
    fun `isQuarantineProtected returns true for quarantined package`() {
        ...
        val result = KillerExecutionHelper.isQuarantineProtected(mockContext, "com.frozen.app")
        assertTrue(result)
    }
    ...
    fun `isQuarantineProtected returns false for non-quarantined package`() {
        ...
        val result = KillerExecutionHelper.isQuarantineProtected(mockContext, "com.example.app")
        assertFalse(result)
    }
